...
# kubectl port-forward service/lidarr -n downloads 7878:80
...
if [[ "${lidarr_eventtype:-}" == "Test" ]]; then
    ...
    PUSHOVER_MESSAGE="Howdy this is a test notification from ${lidarr_instancename:-Radarr}"
fi
...
if [[ "${lidarr_eventtype:-}" == "Download" ]]; then
    ...
    printf -v PUSHOVER_MESSAGE "<b>%s (%s)</b><small>\n%s</small><small>\n\n<b>Client:</b> %s</small><small>\n<b>Quality:</b> %s</small><small>\n<b>Size:</b> %s</small>" \
        "${lidarr_movie_title}" \
        "${lidarr_movie_year}" \
        "${lidarr_movie_overview}" \
        "${lidarr_download_client}" \
        "${lidarr_moviefile_quality}" \
        "$(numfmt --to iec --format "%8.2f" "${lidarr_release_size}")"
    printf -v PUSHOVER_URL "%s/movie/%s" "${lidarr_applicationurl:-localhost}" "${lidarr_movie_tmdbid}"
    printf -v PUSHOVER_URL_TITLE "View movie in %s" "${lidarr_instancename:-Radarr}"
fi
...
if [[ "${lidarr_eventtype:-}" == "ManualInteractionRequired" ]]; then
    ...
    printf -v PUSHOVER_URL_TITLE "View queue in %s" "${lidarr_instancename:-Radarr}"
fi
