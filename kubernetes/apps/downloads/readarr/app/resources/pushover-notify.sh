...
if [[ "${readarr_eventtype:-}" == "Test" ]]; then
    ...
    PUSHOVER_MESSAGE="Howdy this is a test notification from ${readarr_instancename:-Radarr}"
fi
...
if [[ "${readarr_eventtype:-}" == "Download" ]]; then
    ...
    printf -v PUSHOVER_URL_TITLE "View movie in %s" "${readarr_instancename:-Radarr}"
fi
...
if [[ "${readarr_eventtype:-}" == "ManualInteractionRequired" ]]; then
    ...
    printf -v PUSHOVER_URL_TITLE "View queue in %s" "${readarr_instancename:-Radarr}"
fi
